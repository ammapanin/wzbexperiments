...
class BudgetPayment():
    ...
    def remember_frame(self):
        ...
        person = self.idx_data["interview_name"]
    ...
    def show_payment(self):
        ...
        if self.wt == "risk":
            ...
            ptext = payment_text.format(self.wq["interview_name"],
                                        int(a_win),
                                        tomorrow_text)
            winnings = [a_win, "NA"]
        ...
        self.pop_bt.config(command = self.get_comments)
        return None

    def get_comments(self):
        self.payment_frame.pack_forget()
        screen = self.treatment_dic[self.wt]["screen"]
        screen.mainframe.pack_forget()

        screen.qtext.config(text = "")
        variables = ("enumid", "tid", "vid", "interview_wzb.ind.id")
        idx = tuple([self.idx_data.get(v) for v in variables])
        comment_text = "Please enter any comments."
        comment_lab = tk.Label(self.comments_frame,
                               text = comment_text)

        exit_comment = tk.Text(self.comments_frame,
                               width = 50,
                               height = 15,
                               background = "gray")
        ...
        exit_bt = tk.Button(self.comments_frame,
                            text = "End experiment",
                            command = close)
    ...
    def close_window(self, info):
        ...
        comments_path = os.path.join(self.comments_path,
                                     comments_name)

        if os.path.isdir(self.comments_path) == False:
            os.makedirs(self.comments_path)

        with open(comments_path, "a") as commentfile:
