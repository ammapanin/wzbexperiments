...
class CorrectInfo(tk.LabelFrame):
    def __init__(self, master, idx, *args, **kwargs):
        ...
        self.display = DisplayCorrectInfo(self)
...
class AddCorrectInfo:
    def __init__(self, master, text_options):
        ...
        def no_trace(name, index, mode):
            ...
            if on == False:
                ...
                [v.set("") for v in self.entry_vars]
            elif on == True:
    ...
    def validate(self):
        msg = "Please indicate whether the household head's details are correct"
        correction_msgs = ("Please enter the household head's name",
                           "Please enter the rationcard id",
                           "Please enter the election id")

        yes = self.yes_var.get()
        no = self.no_var.get()
        corrections = [no * v.get() for v in self.correction_vars]

        entries = zip(corrections, self.entry_vars)

        empty_entry = [c == 1 and v.get() == "" for c, v, in entries]

        warnings = list()
        if self.participate.get() == 0:
            pass
        else:
            if yes + sum(corrections) == 0:
                warnings.append(msg)
            for e, m in zip(empty_entry, correction_msgs):
                if e == True:
                    warnings.append(m)
