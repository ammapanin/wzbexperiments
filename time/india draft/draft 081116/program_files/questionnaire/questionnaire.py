...
class Questionnaire(tk.Frame):
    def __init__(self, master, **kwargs):
        ...
        additional_tabs = self.stimuli = self.options.get("stimuli")
    ...
    def make_tab_frames(self, title_master, q_master):
        ...
        tab_labs = [tk.Label(label_frame,
                             ...
                             name = txt.translate(string.maketrans("",""),
                                                  string.punctuation))
                    for txt in self.tabs]
        [t.pack(side = "left", padx = 5) for t in tab_labs]
    ...
    def process_csv_line(self, headers, line):
        ...
        try:
            ...
            if line_dic.get("label") == "income_date":
                d = [datetime.datetime.today() + datetime.timedelta(i) for i in range(0, 60)]
                dates = [dt.strftime("%d-%b-%y") for dt in d]
                qdic["options"] = dates
            #conditions
    ...
    def get_structure(self):
        ...
        with open(order_csv) as csvfile:
            ...
            try:
                for idx, (ql, qt, qname) in enumerate(qlines):
                    qlabs.append(ql)
                    qtabs.update({ql:{"idx": idx,
                                      "tab": qt,
                                      "text": qname}})
            except Exception as e:
                print e
                print "The following lines have not been processed:\n"
                print list(qlines)
                print "\n"
        def sort_labs(lab):
            qlab = qtabs.get(lab)
            tab = qlab.get("tab")
            tab_idx = tab_order.index(tab)
            idx = qlab.get("idx")
            return (tab_idx, idx)

        qlabs.sort(key = sort_labs)
        #Check that all tabs have been accounted for
    ...
    def create_tables(self, qdics, qtabs):
        ...
        tables = dict([(t,
                        {"qlabel": t,
                         ...
                         "question_text": qtabs.get(t).get("text") + (" (Please remember to "
                                                                      "scroll right"
                                                                      " and down to complete "
                                                                      "all questions.)"),
                            "conditions": []})
                       for t in tab_names])
