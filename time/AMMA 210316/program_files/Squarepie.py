...
# Next steps - see what is happening for certainty
# Test out new stimuli
# Finagle with timeline
...
class SquarePie(tk.Canvas):
    ...
    def draw_display(self, prob_type):
        ...
        if prob_type == "pie":
            ...
            self.circles, self.ptext = self.draw_circles(10, 10)
    ...
    def draw_circles(self, nx, ny):
        ...
        xy_list = [zip(x1, itertools.cycle((y11,)), x2, itertools.cycle((y22,)))
                   for y11, y22 in yy]
        xy = [c for clist in xy_list for c in clist]

        circles = [self.create_oval(a, b, c, d) for a, b, c, d in xy]
        text = self.draw_probtext()

        def update_probability_circles(p):
            qcol = "dark gray"
            p = int(p * 1)
            [self.itemconfig(i, fill = self.pie_colour) for i in circles[0: p]]
            [self.itemconfig(i, fill = qcol) for i in circles[p:]]
            self.itemconfig(self.ptext, text = "{}%".format(p))
            return None
    ...
    def draw_pie(self):
        ...
        def update_probability_pies(prob):
            ...
            self.itemconfigure(prob_text, text = "{}%".format(p_int))
            self.resize("event")
            return None
    ...
    def draw_amounts(self):
        ...
        amounts = [self.create_text(0, 0,
                                    text = ("{}" + euro).format(7),
                                    font = amtfont) for x in range(0, 4)]
...
class PieScreen(tk.Canvas):
    ...
    def __init__(self, master, prob_type):
        tk.Canvas.__init__(self, master)
    ...
    def draw_timeline(self):
        ...
        timeline = self.create_line(10, 10, 10, 10,
                                    ...
                                    fill = "gray")
        m0 = "Tmrw"
        month_nrs = [0,0.5, 1, 3, 6, 9, 11, 11.5, 12]
