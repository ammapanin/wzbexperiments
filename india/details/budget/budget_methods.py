...
class budgetSetup():
    ...
    def setup_treatment(self, master, n_questions, dims, q_index, texts, base):
        ...
        self.dataPATH = os.path.join(pfilespath,
                                     "data")
        self.identifiersPATH = os.path.join(pfilespath,
                                            "data_identifiers")
    ...
    def data_todic(self):
        ...
        data = (p1, p2,
                ...
                t1, t2,
                a1, a2,
                self.timing, self.start_n2, self.treatment)
    ...
    def write_data(self):
        ...
        identifiers = ("tid", "vid", "wzb.ind.id")
        tid, vid, hid =  [self.ind_data.get(idx) for idx in identifiers]
        id_tags = (self.treatment, tid, vid, hid)

        fname = "{}_{}_{}_{}.csv".format(*id_tags)
        idxname = "budget_{}_{}.csv".format(tid, vid)

        fpath = os.path.join(self.dataPATH, fname)
        idxpath = os.path.join(self.identifiersPATH, idxname)



        if os.path.isdir(self.dataPATH) == False:
            os.makedirs(self.dataPATH)

        if os.path.isdir(self.identifiersPATH) == False:
            os.makedirs(self.identifiersPATH)


        data_cols = ("tid", "vid", "enumid", "wzb.hh.id", "wzb.ind.id",
                     "treatment", "pL", "pR", "nL", "nR",
                     "tL", "tR", "aL", "aR", "timing","start_n2",
                     "date", "time", "version")
        data_colnames = ("qid",) + data_cols

        idx_cols = ("tid", "vid", "enumid", "wzb.hh.id", "wzb.ind.id",
                    "hh_name", "hh_name_corrected", "interviewed_name",
                    "election_id", "ration_nr",
                    "election_corrected", "ration_corrected",
                    "treatment", "date", "time", "version")


        data = [[q] + [dic.get(tag) for tag in data_cols]
                for q, dic in data_tuples]
        individual = [dic.get(tag) for tag in idx_cols]

        with open(fpath, "w") as dfile:
            dwrite = csv.writer(dfile)
            [dwrite.writerow(row)
             for row in [data_colnames] + data]

        with open(idxpath, "a") as idxfile:
            idxwrite = csv.writer(idxfile)
            idxwrite.writerow(individual)

        print "Experiment data written to ", fname
    ...
    def go_next(self):
        ...
        practicemsg =  ("Those were the practice questions."
    ...
    def switch_treatments(self):
        ...
        if tm == "time":
            treattext = self.texts[tm].format(t1[0], t1[1], t2[0], t2[1])
        else:
...
class budgetMethods():
    ...
    def draw_price(self, master, side):
        """"Creates individual price labels
        ...
        ball_font = tkFont.Font(size = 15)
        price.create_text(25, 10,
        ...
        price.create_oval(30, 30, pc, pc, fill = col)
    ...
    def update_allocation(self, event):
        ...
        nb = nr / 2
        n_balls = (50 - nb, nb)
        [c.itemconfig("ball_number", text = "{} balls".format(n))
         for c, n in zip(self.price_objects, n_balls)]
