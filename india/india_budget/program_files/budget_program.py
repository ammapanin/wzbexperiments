...
programspath = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
budgetpath = os.path.dirname(os.path.abspath(__file__))
#print "program path:  ", programspath
sys.path.append(programspath)
...
class Budget(BudgetPayment):
    ...
    def __init__(self, master, dims, n_risk, n_time):
        ...
        self.paymentsPATH = os.path.join(os.path.dirname(programspath),
                                         "payments")
    ...
    def show_instructions(self):
        self.ind_data = dict([(k, v.get())
                            for k, v in self.startscreen.entry_dic.items()])

        print "ind_data", self.ind_data

        [s.ind_data.update(self.ind_data)
